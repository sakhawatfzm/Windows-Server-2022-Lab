...
## Now create namespace one by one. Here I create the namespace for **CEOUser**, and the remaining namespaces are created using the same procedure according to the network topology.

**Create the DFS Namespace one by one. In this example, the namespace for `CEOUser` is created first. Repeat the same process for the remaining departments by following the network topology shown above.**
...
**Review the namespace settings before completing the wizard, then click **Create** to finish the DFS Namespace setup.**
